...
<script>
    $(document).ready(function() {
        ...
        $('#formTambahPeneriman').submit(function(e) {
            ...
            $('#btnText').hide();
            $('#btnSpinner').show();
            $('input[name="barang_id[]"]').each(function(index) {
                var barangId = $(this).val();
                var ambilBarang = $('input[name="ambil_barang_murni[]"]').eq(index).val();
                var hargaSatuan = $('input[name="harga_satuan[]"]').eq(index).val();
                var jumlahHarga = $('input[name="jumlah_harga[]"]').eq(index).val();

                console.log(`Data barang ${index + 1}:`);
                console.log(`Barang ID: ${barangId}`);
                console.log(`Ambil Barang: ${ambilBarang}`);
                console.log(`Harga Satuan: ${hargaSatuan}`);
                console.log(`Jumlah Harga: ${jumlahHarga}`);
            });
            ...
            $.ajax({
                ...
                success: function(response) {
                    // Log data yang dikirim lewat POST
                    console.log('Data yang dikirim lewat POST:', $(this).serialize() + '&id_pengeluaran=' + id_pengeluaran);
...
<script>
    function pilihBarang(barangId, namaBarang, hargaSatuan, jumlahBarang) {
        ...
        // Memilih jumlah_barang dari sisa_stok atau jumlah_barang
        var jumlahBarangValue = isNaN(jumlahBarang) ? 0 : jumlahBarang;
        document.getElementById("jumlah_barang").value = jumlahBarangValue;

        // Menampilkan detail data barang di konsol
        console.log("ID Barang: " + barangId);
        console.log("Nama Barang: " + namaBarang);
        console.log("Harga Satuan: " + hargaSatuan);
        console.log("Jumlah Barang: " + jumlahBarangValue);
