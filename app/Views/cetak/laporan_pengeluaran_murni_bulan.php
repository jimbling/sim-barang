...
<head>
    ...
    <title>Laporan Pengeluaran Murni Bulan_<?= $namaBulan ?>_<?= $tahun ?>
    </title>
        ...
        <div class="table-container page-break-before">
            <table class="table table-border">
                ...
                <tbody>
                    ...
                    <?php else : ?>
                        <?php $i = 1; ?>
                        <?php foreach ($data_pengeluaran as $index => $pengeluaranMurniBT) : ?>
                            <tr>
                                <?php if ($index === 0 || $pengeluaranMurniBT['pengguna_id'] !== $data_pengeluaran[$index - 1]['pengguna_id']) : ?>
                                    <!-- Tampilkan data hanya sekali dalam satu kelompok pengguna_id -->
                                    <th class="text-center" scope="row" style="vertical-align: middle; " rowspan="<?= count(array_filter($data_pengeluaran, function ($item) use ($pengeluaranMurniBT) {
                                                                                                                        return $item['pengguna_id'] === $pengeluaranMurniBT['pengguna_id'];
                                                                                                                    })); ?>">
                                        <?= $i++; ?>
                                    </th>
                                    <td style="text-align: center; vertical-align: middle; " rowspan="<?= count(array_filter($data_pengeluaran, function ($item) use ($pengeluaranMurniBT) {
                                                                                                            return $item['pengguna_id'] === $pengeluaranMurniBT['pengguna_id'];
                                                                                                        })); ?>">
                                        <?php
                                        $tanggal_penggunaan = \CodeIgniter\I18n\Time::parse($pengeluaranMurniBT['tanggal_penggunaan'])
                                    ...
                                    <td style="text-align: left; vertical-align: middle; " rowspan="<?= count(array_filter($data_pengeluaran, function ($item) use ($pengeluaranMurniBT) {
                                                                                                        return $item['pengguna_id'] === $pengeluaranMurniBT['pengguna_id'];
                                                                                                    })); ?>">
                                        <?= $pengeluaranMurniBT['nama_pengguna_barang']; ?>
                                    </td>
                                    <td style="text-align: left; vertical-align: middle; " rowspan="<?= count(array_filter($data_pengeluaran, function ($item) use ($pengeluaranMurniBT) {
                                                                                                        return $item['pengguna_id'] === $pengeluaranMurniBT['pengguna_id'];
                                                                                                    })); ?>">
                                        <?= $pengeluaranMurniBT['keperluan']; ?>
                                    </td>
                                <?php endif; ?>

                                <!-- Tampilkan data yang berulang -->
                                <td style="text-align: left; vertical-align: middle; ">
                                    <?= $pengeluaranMurniBT['nama_barang']; ?>
                                </td>
                                <td style="text-align: left; vertical-align: middle; ">
                                    <?= $pengeluaranMurniBT['ambil_barang_murni']; ?> <?= $pengeluaranMurniBT['satuan']; ?>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
