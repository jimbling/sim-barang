...
<head>
    ...
    <title>Laporan Pengeluaran Tahun_<?= $tahun ?>
    </title>
        ...
        <div class="table-container page-break-before">
            <table class="table table-border">
                ...
                <tbody>
                    ...
                    <?php else : ?>
                        <?php $i = 1; ?>
                        <?php foreach ($data_pengeluaranTahun as $index => $peminjaman) : ?>
                            <tr>
                                <?php if ($index === 0 || $peminjaman['peminjaman_id'] !== $data_pengeluaranTahun[$index - 1]['peminjaman_id']) : ?>
                                    <!-- Tampilkan data hanya sekali dalam satu kelompok peminjaman_id -->
                                    <th class="text-center" scope="row" style="vertical-align: middle; " rowspan="<?= count(array_filter($data_pengeluaranTahun, function ($item) use ($peminjaman) {
                                                                                                                        return $item['peminjaman_id'] === $peminjaman['peminjaman_id'];
                                                                                                                    })); ?>">
                                        <?= $i++; ?>
                                    </th>
                                    <td style="text-align: left; vertical-align: middle; " rowspan="<?= count(array_filter($data_pengeluaranTahun, function ($item) use ($peminjaman) {
                                                                                                        return $item['peminjaman_id'] === $peminjaman['peminjaman_id'];
                                                                                                    })); ?>">
                                        <?= $peminjaman['kode_pinjam']; ?>
                                    </td>
                                    <td style="text-align: center; vertical-align: middle; " rowspan="<?= count(array_filter($data_pengeluaranTahun, function ($item) use ($peminjaman) {
                                                                                                            return $item['peminjaman_id'] === $peminjaman['peminjaman_id'];
                                                                                                        })); ?>">
                                        <?php
